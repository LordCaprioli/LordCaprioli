<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Portfólio - Lucas Caprioli de Sena</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            margin: 0;
            padding: 0;
            background-color: #f4f4f4;
        }
        header {
            background-color: #333;
            color: white;
            text-align: center;
            padding: 20px 0;
        }
        header h1 {
            margin: 0;
        }
        main {
            max-width: 800px;
            margin: 20px auto;
            background: white;
            padding: 20px;
            border-radius: 8px;
            box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1);
        }
        .photo {
            text-align: center;
        }
        .photo img {
            width: 150px;
            height: 150px;
            border-radius: 50%;
            border: 2px solid #333;
        }
        h2 {
            border-bottom: 2px solid #333;
            padding-bottom: 5px;
            margin-bottom: 15px;
            color: #333;
        }
        ul {
            list-style-type: none;
            padding: 0;
        }
        ul li {
            margin-bottom: 10px;
        }
        footer {
            text-align: center;
            padding: 10px;
            background: #333;
            color: white;
            margin-top: 20px;
        }
    </style>
</head>
<body>
    <header>
        <h1>Lucas Caprioli de Sena</h1>
        <p>Atendente Online | Suporte ao Consumidor</p>
    </header>
    <main>
        <div class="photo">
            <img src="your-photo.jpg" alt="Foto de Lucas Caprioli">
        </div>
        <section>
            <h2>Objetivo</h2>
            <p>Atuar na área de atendimento e suporte ao consumidor, oferecendo soluções eficientes e um serviço de qualidade, visando à satisfação e fidelização dos clientes, além de contribuir para o crescimento da empresa.</p>
        </section>
        <section>
            <h2>Experiência</h2>
            <ul>
                <li><strong>Sena Lan-House (Assistente de Loja) 2016/2024</strong></li>
                <li>Atendimento ao público e por telefone, abertura e fechamento de caixa, controle e analista de estoque, suporte técnico intermediário, manutenção periódica em computadores, organização e limpeza do local.</li>
            </ul>
        </section>
        <section>
            <h2>Formação</h2>
            <ul>
                <li>Ensino Médio ENCCEJA - 2020</li>
            </ul>
        </section>
        <section>
            <h2>Qualificações</h2>
            <ul>
                <li>Pacote Office Avançado</li>
                <li>Informática Avançada</li>
                <li>Experiência com Hardware e Software</li>
            </ul>
        </section>
        <section>
            <h2>Competências</h2>
            <ul>
                <li>Proativo</li>
                <li>Pontual</li>
                <li>Trabalho em equipe</li>
                <li>Rápido aprendizado</li>
            </ul>
        </section>
        <section>
            <h2>Informações de Contato</h2>
            <ul>
                <li><strong>Telefone:</strong> (51) 98204-5571</li>
                <li><strong>E-mail:</strong> sena-boy@hotmail.com</li>
                <li><strong>Endereço:</strong> Avenida Paraguassu 2146, Centro</li>
            </ul>
        </section>
    </main>
    <footer>
        <p>Lucas Caprioli de Sena - Disponível para início imediato.</p>
    </footer>
</body>
</html>
